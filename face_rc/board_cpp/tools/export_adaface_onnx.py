#!/usr/bin/env python3
"""Export MeetEye AdaFace checkpoint to ONNX.

The exported model keeps only the 512D L2-normalized embedding output. Input is
float32 NCHW [1, 3, 112, 112] and should already be normalized with
(pixel / 255 - 0.5) / 0.5, matching the C++ board runtime.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="AdaFace backbone name used by mytest/face_rec/AdaFace/net.py",
    )
    parser.add_argument(
        "--checkpoint",
        default="face_rc/board_cpp/models/adaface_ir50_webface4m.ckpt",
        help="AdaFace .ckpt path",
    )
    parser.add_argument(
        "--output",
        default="face_rc/board_cpp/models/adaface_ir50_webface4m_112.onnx",
...
def main() -> None:
    ...
    print(f"[AdaFaceExport] model_name: {args.model_name}")
    print(f"[AdaFaceExport] checkpoint: {checkpoint}")
