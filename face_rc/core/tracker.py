...
try:
    ...
    try:
        from core.boundary_matcher import BoundaryCrossingTracker
        BOUNDARY_MATCHER_AVAILABLE = True
    except ImportError:
        BoundaryCrossingTracker = None
...
class HybridSortTracker:
    ...
    def update(self, detections: List[Dict]) -> List[Dict]:
        ...
        profile: Dict[str, Any] = {
            ...
            "prev_active": len(self._prev_active_ids),
            "feat_cache": len(self._feat_cache),
            "meta_cache": len(self._meta_cache),
            "bbox_cache": len(self._bbox_size_cache),
            "temp_map": len(self.temp_id_map),
            "public_map": len(self._public_id_map),
            "boundary_enabled": bool(self.enable_boundary_matching and self.boundary_tracker is not None),
            "with_reid": bool(self._with_reid),
            "smooth_bbox": bool(self._smooth_bbox),
            "kalman_bbox": bool(self.kalman_bbox),
            "coast_frames": int(self.coast_frames),
        }

        def mark_profile(name: str) -> None:
            nonlocal profile_last
            now = time.perf_counter()
            profile[name] = (now - profile_last) * 1000.0
            profile_last = now

        # ① 边界匹配帧头预处理 ────────────────────────────────────────────────
        if self.enable_boundary_matching and self.boundary_tracker is not None:
            self.boundary_tracker.pre_process(self.frame_id)
        mark_profile("boundary_pre")
        ...
        det_boxes_iou = (
            np.asarray([d['bbox'] for d in detections], dtype=np.float64)
            if detections else np.empty((0, 4), dtype=np.float64)
        )
        mark_profile("prepare_input")
        ...
        needs_feature_guard = self._with_reid or any(d.get("feature") is not None for d in detections)
        if needs_feature_guard and len(dets_np) > 1:
            for _a in range(len(det_boxes_iou)):
                ious = box_iou_batch(det_boxes_iou[_a], det_boxes_iou[_a + 1:])
                hit = np.nonzero(ious > _OVERLAP_IOU_THRESH)[0]
                if hit.size:
                    _contaminated_det_indices.add(_a)
                    for _b in hit:
                        _contaminated_det_indices.add(int(_a + 1 + _b))
        profile["contaminated"] = len(_contaminated_det_indices)
        mark_profile("overlap_scan")
        ...
        profile["current_active"] = len(current_active_ids)
        profile["lost_ids"] = len(lost_ids)
        profile["inner_trackers"] = len(self._inner.trackers)
        mark_profile("lost_maintenance")
        ...
        profile["reverse_iou_pairs"] = 0
        if detections and len(online_targets) > 0:
            pairs: List[Tuple[float, int, int]] = []
            for ti, row in enumerate(online_targets):
                ious = box_iou_batch(row[:4], det_boxes_iou)
                for di in np.nonzero(ious > 0.3)[0]:
                    pairs.append((float(ious[di]), ti, int(di)))
            profile["reverse_iou_pairs"] = len(pairs)
            pairs.sort(key=lambda x: -x[0])
        ...
        for ti, row in enumerate(online_targets):
            ...
            feature_ms += (time.perf_counter() - _sub_t) * 1000.0
            ...
            if (is_new_track
                ...
                boundary_new_ms += (time.perf_counter() - _sub_t) * 1000.0
                if matched_id is not None:
            ...
            smooth_ms += (time.perf_counter() - _sub_t) * 1000.0
            ...
            output_detections.append(det_out)
        profile["feature_ema"] = feature_ms
        profile["boundary_new"] = boundary_new_ms
        profile["bbox_smoothing"] = smooth_ms
        mark_profile("output_build")
        ...
        if self.kalman_bbox or self.coast_frames > 0:
            ...
            for trk in self._inner.trackers:
                ...
                coasting_added += 1
        profile["coasting_added"] = coasting_added
        mark_profile("coast")
        ...
        mark_profile("id_debug")

        inner_profile = getattr(self._inner, "last_profile", {}) or {}
        assignment_profile = inner_profile.get("assignment", {}) or {}
        profile["inner_total"] = float(inner_profile.get("total", profile.get("inner_update", 0.0)))
        profile["inner_preprocess"] = float(inner_profile.get("preprocess", 0.0))
        profile["inner_predict"] = float(inner_profile.get("predict", 0.0))
        profile["inner_state_vectors"] = float(inner_profile.get("state_vectors", 0.0))
        profile["inner_assoc_first"] = float(inner_profile.get("assoc_first", 0.0))
        profile["inner_update_first"] = float(inner_profile.get("update_first", 0.0))
        profile["inner_assoc_byte"] = float(inner_profile.get("assoc_byte", 0.0))
        profile["inner_assoc_ocr"] = float(inner_profile.get("assoc_ocr", 0.0))
        profile["inner_mark_unmatched"] = float(inner_profile.get("mark_unmatched", 0.0))
        profile["inner_new_tracks"] = float(inner_profile.get("new_tracks", 0.0))
        profile["inner_collect"] = float(inner_profile.get("collect_cleanup", 0.0))
        profile["assignment_ms"] = float(assignment_profile.get("time_ms", 0.0))
        profile["assignment_max_ms"] = float(assignment_profile.get("max_ms", 0.0))
        profile["assignment_calls"] = int(assignment_profile.get("calls", 0))
        profile["assignment_shapes"] = assignment_profile.get("shapes", [])
        profile["inner_n_input"] = int(inner_profile.get("n_input", 0))
        profile["inner_n_high"] = int(inner_profile.get("n_high", 0))
        profile["inner_n_second"] = int(inner_profile.get("n_second", 0))
        profile["inner_matches_first"] = int(inner_profile.get("matches_first", 0))
        profile["inner_matches_byte"] = int(inner_profile.get("matches_byte", 0))
        profile["inner_matches_ocr"] = int(inner_profile.get("matches_ocr", 0))
        profile["inner_unmatched_dets"] = int(inner_profile.get("unmatched_dets_after", 0))
        profile["inner_unmatched_trks"] = int(inner_profile.get("unmatched_trks_after", 0))
        profile["created_tracks"] = int(inner_profile.get("created_tracks", 0))
        profile["output"] = len(output_detections)
        profile["total"] = (time.perf_counter() - profile_t0) * 1000.0
