...
FastAPI 应用 + 所有路由
  ...
  - POST /record/start     : 开始录制原始+推理双路视频到板端
...
app = FastAPI(title="MeetEye RK3588 Edge WebUI")
...
async def video_infer():
    return StreamingResponse(_mjpeg_gen_infer(),
                              media_type="multipart/x-mixed-replace; boundary=frame")
...
def record_start():
    with state.record_lock:
        ...
        original_path = f"recordings/original_{ts}.mp4" if state.save_original_video else None
