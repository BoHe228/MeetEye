...
SRC="${ROOT_DIR}/face_rc/tools/rknn_run_benchmark.cpp"
OUT_DIR="${ROOT_DIR}/face_rc/tools/bin"
OUT="${OUT_DIR}/rknn_run_benchmark"
...
if [[ ! -f "${RKNN_INCLUDE_DIR}/rknn_api.h" ]]; then
  ...
  echo "Expected: ${RKNN_API_DIR}/include/rknn_api.h" >&2
  echo "Please sync RKSDK/rknn-toolkit2/rknpu2/runtime/Linux/librknn_api/include to this board." >&2
  exit 1
fi
...
if [[ -z "${RKNN_RT_LIB}" || ! -f "${RKNN_RT_LIB}" ]]; then
  ...
  echo "Try: find /usr /lib \"$VIRTUAL_ENV\" -name librknnrt.so 2>/dev/null" >&2
  exit 1
fi

mkdir -p "${OUT_DIR}"
g++ -O3 -DNDEBUG -std=c++14 \
  -I"${RKNN_INCLUDE_DIR}" \
  "${SRC}" \
  "${RKNN_RT_LIB}" \
  -Wl,-rpath,'$ORIGIN/lib' \
